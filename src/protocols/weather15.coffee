# weather15 has been derived from weather12 as the the protocol of the Globaltronics GT-WT-01 sensor
# appears to have changed with newer makes sold on ebay. Compared to weather12, the values are shifted
# by 4-bits to the right as the id 12-bits instead of 8-bits. Moreover, the humidity is an 8-bit value
# instead of 7-bit.
...
H: 7bit     73% (1001 001 = 73)
      ...
      if helper.binaryToNumber(binary, 12, 12) == 1
        lowBattery = true
      else
        lowBattery = false
      return result = {
