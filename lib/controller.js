var doesProtocolMatch, e, file, files, fs, helper, myprotocols, p, protocols, sortIndices, this_path, _i, _j, _len, _len1,
...
protocols = ['weather1', 'weather2', 'weather3', 'weather4', 'weather5', 'weather6', 'weather7', 'weather8', 'weather9', 'weather10', 'weather11', 'weather12', 'weather13', 'switch1', 'switch2', 'switch3', 'switch4', 'switch5', 'switch6', 'switch7', 'switch8', 'switch9', 'switch10', 'switch11', 'switch12', 'switch13', 'switch14', 'switch15', 'switch16', 'switch17', 'switch18', 'switch19', 'switch20', 'switch21', 'switch22', 'rolling1', 'dimmer1', 'dimmer2', 'pir1', 'pir2', 'pir3', 'pir4', 'pir5', 'contact1', 'contact2', 'contact3', 'generic', 'generic2', 'alarm1', 'alarm2', 'led1', 'led2', 'led3', 'led4', 'doorbell1', 'doorbell2', 'awning1', 'awning2', 'shutter1', 'shutter3', 'shutter4', 'shutter5'];
...
for (_i = 0, _len = files.length; _i < _len; _i++) {
  ...
  if (__indexOf.call(file, '~') >= 0) {
    continue;
  }
  if (__indexOf.call(protocols, file) >= 0) {
...
myprotocols = myprotocols.map((function(_this) {
  return function(p) {
    return require("./protocols/user_protocols/" + p)(helper);
  };
})(this));

for (_j = 0, _len1 = myprotocols.length; _j < _len1; _j++) {
  p = myprotocols[_j];
  protocols.push(p);
}
...
sortIndices = function(array) {
  var i, indices, j, tuple, tuples, _k, _l, _len2, _len3;
  tuples = new Array(array.length);
  for (i = _k = 0, _len2 = array.length; _k < _len2; i = ++_k) {
    e = array[i];
    tuples[i] = [e, i];
  }
  ...
  for (j = _l = 0, _len3 = tuples.length; _l < _len3; j = ++_l) {
    tuple = tuples[j];
    indices[tuple[1]] = j;
  }
  return indices;
...
module.exports = {
  ...
  compressTimings: function(timings) {
    var bucket, buckets, counts, hasMatch, i, j, pulses, sums, timing, _k, _l, _len2, _len3, _len4, _m;
    pulses = '';
    buckets = [];
    sums = [];
    counts = [];
    for (i = _k = 0, _len2 = timings.length; _k < _len2; i = ++_k) {
      timing = timings[i];
      hasMatch = false;
      for (j = _l = 0, _len3 = buckets.length; _l < _len3; j = ++_l) {
        bucket = buckets[j];
    ...
    for (j = _m = 0, _len4 = buckets.length; _m < _len4; j = ++_m) {
      bucket = buckets[j];
  ...
  decodePulses: function(pulseLengths, pulses) {
    var err, fixed, results, values, _k, _len2;
    results = [];
    for (_k = 0, _len2 = protocols.length; _k < _len2; _k++) {
      p = protocols[_k];
